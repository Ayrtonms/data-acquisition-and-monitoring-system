	...
	public void trigger() {
		...
		if (getModod() != 0) {
			if (!Utils.checkData(getData1())) {
				ok = false;
				context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "DATE 1 IS INVALID!" + "", ""));
			}

			if (getModod() == 4 && !Utils.checkData(getData2())) {
				ok = false;
				context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "DATE 2 IS INVALID!" + "", ""));
			}
		}
		...
		if (getModoh() != 0) {
			if (!Utils.checkHora(getHora1())) {
				ok = false;
				context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "HOUR 1 IS INVALID!" + "", ""));
			}

			if (getModoh() == 4 && !Utils.checkHora(getHora2())) {
				ok = false;
				context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "HOUR 2 IS INVALID!" + "", ""));
			}
		}

		if (getQtd() <= 0) {
			ok = false;
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "INVALID AMOUNT!", ""));
		}

		if (getQtd() > 100) {
			ok = false;
			context.addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "AMOUNT SPECIFIED IS TOO HIGH!", ""));
		}

		if (getAgr() != 0 && getEixo() != 0) {
			ok = false;
			context.addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "GROUP CHARTS REQUIRE DATE ON X AXIS!", ""));
		}
